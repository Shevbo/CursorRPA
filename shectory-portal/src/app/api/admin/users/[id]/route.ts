...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  ...
  const sess = currentPortalSessionFromRequest(req);
  if (sess?.role !== "superadmin") {
    return NextResponse.json({ error: "Только superadmin может удалять пользователей" }, { status: 403 });
  }
  ...
  if (target.role === "superadmin") {
    const cnt = await prisma.portalUser.count({ where: { role: "superadmin" } });
    if (cnt <= 1) {
      return NextResponse.json({ error: "Нельзя удалить последнего superadmin" }, { status: 400 });
    }
  }

  // Prevent deleting yourself
  const meEmail = sess.email ? normalizeEmail(sess.email) : "";
  if (meEmail && normalizeEmail(target.email) === meEmail) {
    return NextResponse.json({ error: "Нельзя удалить самого себя" }, { status: 400 });
  }

  // Delete related records first (cascade manually if FK constraints)
  await prisma.portalEmailCode.deleteMany({ where: { userId: params.id } });
  await prisma.portalNotification.deleteMany({ where: { userId: params.id } });
  await prisma.portalUserProfile.deleteMany({ where: { userId: params.id } });
