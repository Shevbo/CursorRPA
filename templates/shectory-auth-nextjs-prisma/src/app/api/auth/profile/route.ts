...
export async function GET() {
  ...
  if (!session?.user?.id) return NextResponse.json({ profile: null }, { status: 200 });
  ...
  if (profile.created_at) {
    profile_number = await prisma.profiles.count({
      where: { created_at: { lte: profile.created_at } },
    });
  }

  return NextResponse.json({ profile: { ...profile, profile_number } });
}
...
export async function PATCH(request: Request) {
  ...
  const { full_name, phone, email, preferred_location, email_notifications_enabled } = (body as {
    full_name?: unknown;
    phone?: unknown;
    email?: unknown;
  ...
  if (typeof full_name === "string") updatesProfile.full_name = full_name.trim() || null;
  if (typeof phone === "string") updatesProfile.phone = phone.trim() || null;
  if (typeof preferred_location === "string") updatesProfile.preferred_location = preferred_location.trim() || null;
  ...
  if (typeof email === "string") {
    ...
    if (!normalizedEmail) return NextResponse.json({ error: "Email не может быть пустым" }, { status: 400 });
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (Object.keys(updatesUser).length > 0) {
        await tx.users.update({ where: { id: session.user.id }, data: updatesUser });
      }
    ...
    if (result.created_at) {
      profile_number = await prisma.profiles.count({ where: { created_at: { lte: result.created_at } } });
    }
    return NextResponse.json({ profile: { ...result, profile_number } });
  } catch (err) {
...
export async function DELETE() {
  ...
  try {
    ...
    return NextResponse.json({ error: "Не удалось удалить профиль. Попробуйте позже." }, { status: 500 });
  }
  return NextResponse.json({ ok: true });
}
