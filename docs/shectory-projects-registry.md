...
| ourdiary    | Наш дневник (ourdiary) | dev         | `-`                        | `/home/shectory/workspaces/ourdiary`    | `git@github.com:Shevbo/ourdiary.git`                                                    | ok                         | prod на Hoster (по RUNBOOK)       | Next.js 16, Prisma, NextAuth | Деплой: `./scripts/deploy-project.sh ourdiary hoster`. На **hoster** клон в **`~/ourdiary`**, в репо **`scripts/deploy.sh`** (pm2 `ourdiary` или `OURDIARY_SYSTEMD_USER_UNIT`). |
