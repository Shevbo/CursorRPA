...
| ourdiary    | Наш дневник (ourdiary) | dev         | `http://127.0.0.1:3002` (hoster, pm2) | `/home/shectory/workspaces/ourdiary`    | `git@github.com:Shevbo/ourdiary.git`                                                    | ok                         | prod на Hoster (Postgres локально) | Next.js 16, Prisma, NextAuth | Первичная установка на hoster: **`scripts/bootstrap-hoster.sh`**. Деплой обновлений: **`deploy-project.sh ourdiary hoster`**. PM2: **`ecosystem.config.cjs`**, порт **3002**. |
