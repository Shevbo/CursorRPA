...
- Почта для кодов регистрации / сброса пароля (опционально): `SMTP_HOST`, `AUTH_EMAIL_FROM`, при необходимости `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`, `SMTP_SECURE`; при самоподписанном TLS почтовика: `SMTP_TLS_REJECT_UNAUTHORIZED=false`; принудительно только журнал процесса: `AUTH_CODE_DELIVERY_MODE=log`
