...
- **Почта (Poste.io на этой VDS):** Docker-контейнер вроде **`mail-poste-poste-1`** (`analogic/poste.io`), внутри **`/var/log` → `/data/log`** (volume). Если ночной cron с **`logrotate`** падает с текстом **«parent directory has insecure permissions»**, обычно у **`/data/log`** или подкаталогов права **777**. Исправление с **`docker exec -u root <контейнер>`:** `chmod 755 /data/log`, затем **`find /data/log -type d -exec chmod 755 {} \;`**, для веб-почты **`chmod 755 /data/roundcube/logs`**. Проверка: **`logrotate -d /etc/logrotate.conf`** без ошибок про insecure; реальный запуск **`/usr/sbin/logrotate /etc/logrotate.conf --state=/data/var/logrotate.status`** — код выхода **0**.
- **DNS почты для `shectory.ru`:** письма с **`portal@shectory.ru`** к внешним ящикам (Mail.ru и т.д.) нормально проходят с **SPF + DKIM** в публичной зоне. Готовый текст записей (SPF, DKIM селектор **`poste`**, пример DMARC) — **[docs/dns-mail-shectory-ru.txt](dns-mail-shectory-ru.txt)**. **DKIM** на Poste.io подписывается плагином Haraka **`mailauth/dkim_sign`**: ключи на томе в **`/data/haraka-dkim/<домен>/private`** и **`selector`**, в runtime — symlink в **`/opt/haraka-submission/config/dkim/<домен>/`**. После пересоздания контейнера symlink пропадает: **`bash scripts/poste-haraka-dkim-init.sh`** с VDS. Проверки: **`dig TXT shectory.ru`**, **`dig TXT poste._domainkey.shectory.ru`**, **`dig MX shectory.ru`**. В логах портала исходящая попытка: **`[AUTH_EMAIL] SMTP sent`**. В сыром письме должен быть заголовок **DKIM-Signature** с **d=shectory.ru** (и **s=poste** при использовании селектора выше).
...
- **Prisma Studio «наружу» (через HTTPS приложения)** — у прикладных проектов на hoster с встроенным Studio (как **ourdiary**: страница **`/admin/prisma-studio`**, BFF **`POST /api/admin/studio`**) UI и API доступны с **того же публичного origin**, что и приложение (например `https://ourdiary.shectory.ru/...`), за **nginx/TLS** на VDS. Это **не** открытый Postgres в интернет: запросы идут в Next.js, **сессия администратора приложения** (роли `ADMIN` / `SUPERADMIN`) обязательна; BFF выполняет SQL от имени `DATABASE_URL` только после проверки роли. Обычные REST/API маршруты приложения на Prisma тем же образом «снаружи» — через тот же домен и механизмы авторизации. Карточка проекта на портале Shectory может содержать ссылку **`prismaStudioUrl`** → `{appUrl}/admin/prisma-studio` (см. `prisma/seed.ts` в монолите). Подробнее по конкретному приложению — `RUNBOOK.md` в репозитории проекта.
...
- **Коды по почте (регистрация, сброс пароля)** — `/login`: сценарий «Забыли пароль» двухшаговый в UI; API `POST /api/auth/forgot/request-code` и `.../confirm`. Реальная отправка письма: задать в `.env` портала **`SMTP_HOST`**, **`AUTH_EMAIL_FROM`**, при необходимости **`SMTP_PORT`**, **`SMTP_USER`**, **`SMTP_PASSWORD`**, **`SMTP_SECURE=true`** (или порт 465); для почтовика с **самоподписанным TLS** — **`SMTP_TLS_REJECT_UNAUTHORIZED=false`** (иначе nodemailer отказывается в TLS). Реализация: `nodemailer` в `shectory-portal/src/lib/portal-auth.ts` (`deliverAuthCode`). Если SMTP не задан или отправка упала — код в stdout/journal (`[AUTH_EMAIL LOG]`). Принудительно только лог: **`AUTH_CODE_DELIVERY_MODE=log`**. В не-production ответ API может содержать **`debugCode`** для отладки.
...
- **Telegram (регулярные сообщения + алерты)**:
  ...
  - **SSH на Pi (как задать `PI_MONITOR_SSH`)**:
    - Норма для бота и скриптов: **OpenSSH** с **`BatchMode=yes`** и входом **по ключу** (без интерактива). Пример формы: `ssh -o BatchMode=yes -o ConnectTimeout=5 <user>@<pi-host>`, где `<pi-host>` — WireGuard IP **`10.66.0.2`** или алиас **`shevbo-pi`** в `~/.ssh/config` на рабочей станции Shectory.
...
- **Публичный URL Syslog UI (`https://syslog.shectory.ru`)**:
  ...
  - **UDP syslog с интернета на Pi (`syslog.shectory.ru:5514`):** тот же публичный IP; на **`shectory-work`** в **`iptables`**: **DNAT** `udp/5514` → **`10.66.0.2:5514`** и **SNAT** для источников вне **`10.66.0.0/24`** на выходе в **`wg0`**: **`SNAT --to-source 10.66.0.1`** (иначе часть клиентов не доставляет пакеты до процесса на Pi при срабатывании DNAT). Правила хранить в **`/etc/iptables/rules.v4`** через **`netfilter-persistent`**. Локальный экземпляр **syslog-srv на VDS** (если есть) — **`SYSLOG_PORT=0`**, чтобы порт **5514** на хосте был свободен. Идентификация отправителей в основном по **hostname в теле syslog**, не по IP сокета (после SNAT с внешних адресов на Pi источник выглядит как **`10.66.0.1`**).
  - **Версия продукта:** **1.0.0 от 03.04.2026 (R) Shectory** — первая стабильная редакция репозитория **syslog-srv**; см. **`CHANGELOG.md`** и **`README.md`** в репозитории.
