...
- После **завершения правок по задаче** агент **сам** фиксирует результат в git и **сам** выкладывает на целевое окружение: выполняет осмысленный **`git commit`**, при необходимости **`git push`**, затем деплой принятыми в репозитории скриптами (например **`scripts/deploy-shectory-portal.sh`** для портала, аналоги для других сервисов) и при необходимости рестарт systemd user/unit, если это часть стандартного контура деплоя.
