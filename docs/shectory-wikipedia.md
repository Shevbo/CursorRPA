...
- **DNS почты для `shectory.ru`:** если письма с **`portal@shectory.ru`** не видны получателю на Mail.ru/Gmail и т.п., хотя в логах портала есть **`[AUTH_EMAIL] SMTP sent`**, почти всегда не хватает **`TXT`** у домена (**SPF**, позже **DKIM/DMARC** из веб-админки Poste → те же записи у регистратора). Без SPF крупные провайдеры часто **режут входящие** или кладут в спам. Проверки: **`dig TXT shectory.ru`**, **`dig MX shectory.ru`** — имя **`mail`** в MX не должно превращаться в **`mail.shectory.ru.shectory.ru`** (опечатка зоны); для исходящих обычно нужна строка вида **`v=spf1 ip4:83.69.248.77 a mx ... -all`** или то, что показывает Poste для вашего отправителя.
